#!/usr/bin/env bash

python eval_single_obj.py --dataset=scannet40 \
               --dataset_mode=single_obj \
               --scan_folder=data/ScanNet/scans \
               --val_list=data/ScanNet/single/object_ids.npy \
               --val_list_classes=data/ScanNet/single/object_classes.txt \
               --output_dir=results/ScanNet_single \
               --checkpoint=weights/checkpoint1099.pth

CUDA_VISIBLE_DEVICES=5 python eval_single_obj.py --dataset=scannet40 \
               --dataset_mode=single_obj \
               --scan_folder=/mnt/cloud_disk/lxp/data/ScanNet/scans \
               --val_list=/mnt/cloud_disk/lxp/data/ScanNet/single/object_ids.npy \
               --val_list_classes=/mnt/cloud_disk/lxp/data/ScanNet/single/object_classes.txt \
               --output_dir=results/ScanNet_single \
               --checkpoint=/mnt/cloud_disk/lxp/AGILE3D-main/output/2024-10-14-12-36-48/checkpoint1079.pth