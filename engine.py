...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, batched_inputs in enumerate(metric_logger.log_every(data_loader, print_freq, header,rank =rank)):
        ...
        full_scene_click_idx = copy.deepcopy(click_idx)
        ...
        for idx in range(batch_idx.max()+1):
            ...
            sample_full_labels_new = torch.zeros(sample_labels.shape[0], device=device)
            ...
            for i, obj_id in enumerate(valid_obj_idxs):
                obj_mask = sample_labels == obj_id
                sample_full_labels_new[obj_mask] = i+1

                full_scene_click_idx[idx][str(i+1)] = []

            click_idx[idx]['0'] = []
            full_scene_click_idx[idx]['0'] = []
            labels_new.append(sample_labels_new)
            full_labels_new.append(sample_full_labels_new)

        click_time_idx = copy.deepcopy(click_idx)
        full_scene_click_time_idx = copy.deepcopy(full_scene_click_idx)
            ...
            while current_num_iter <= num_forward_iters:
                ...
                if current_num_iter == 0:
                    for idx in range(batch_idx.max()+1):
                        ...
                        sample_full_labels = full_labels_new[idx]
                        sample_raw_coords = raw_coords[sample_mask]
                        new_clicks, new_clicks_num, new_click_pos, new_click_time = get_simulated_clicks(
                            sample_pred, sample_full_labels, sample_raw_coords, current_num_iter, training=True)
                        full_scene_click_idx[idx], full_scene_click_time_idx[idx] = extend_clicks(
                            full_scene_click_idx[idx], full_scene_click_time_idx[idx], new_clicks, new_click_time)
        ...
        full_instance_outputs = model.forward_mask(pcd_features, aux, coordinates, pos_encodings_pcd, 
                                     click_idx=full_scene_click_idx, click_time_idx=full_scene_click_time_idx)
        ...
        click_losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)

        full_instance_click_weights = cal_click_loss_weights(coords[:,0], raw_coords, torch.cat(full_labels_new), full_scene_click_idx)
        full_instance_loss_dict = criterion(full_instance_outputs, full_labels_new, full_instance_click_weights)
        full_instance_losses = sum(full_instance_loss_dict[k] * weight_dict[k] for k in full_instance_loss_dict.keys() if k in weight_dict)

        losses = click_losses + full_instance_losses
        ...
        with torch.no_grad():
            ...
            metric_logger.update(loss=losses.item(), **loss_dict_reduced_scaled)
            metric_logger.update(clicks_loss=click_losses.item())
            metric_logger.update(full_instance_loss=full_instance_losses.item())
        ...
        if ((i + 1) % 100 == 0) and rank==0:
            wandb.log({
                ...
                "train/clicks_loss": metric_logger.meters['clicks_loss'].avg,
                "train/full_instance_loss": metric_logger.meters['full_instance_loss'].avg,
