...
def get_args_parser():
    ...
    parser.add_argument('--scan_folder', default='/mnt/cloud_disk/lxp/data/ScanNet/scans', type=str)
    parser.add_argument('--train_list', default='/mnt/cloud_disk/lxp/data/ScanNet/train_list.json', type=str)
    parser.add_argument('--val_list', default='/mnt/cloud_disk/lxp/data/ScanNet/val_list.json', type=str)
...
def main(rank, world_size, args):
    ...
    data_loader_train = DataLoader(dataset_train, batch_size=args.batch_size, pin_memory=True, 
                            num_workers=0, drop_last=False, collate_fn=collation_fn_train, 
                            shuffle=False, sampler=sampler_train)
    # data_loader_val = DataLoader(dataset_val, batch_size=args.val_batch_size, pin_memory=True, 
    #                         num_workers=0, drop_last=False, collate_fn=collation_fn_val, 
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, train_total_iter = train_one_epoch(
            model, criterion, data_loader_train, optimizer, device, epoch, train_total_iter, args.clip_max_norm,rank=rank)
        ...
        if (epoch + 1) % args.val_epochs == 0 and rank == 0:
             
            test_stats = evaluate(
                model, criterion, data_loader_val, args, epoch, device, rank=rank
            )
