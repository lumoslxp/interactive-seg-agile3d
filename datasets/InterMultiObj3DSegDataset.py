...
class InterMultiObj3DSegDataset(Dataset):
    ...
    def get_instance_info(self, xyz, instance_ids):
        ...
        '''
        xyz = xyz.float()
        centers = np.ones((xyz.shape[0], 3), dtype=np.float32) * -1   
        ...
        return {"ids": instance_ids, "center": centers}
    ...
    def __getitem__(self, i):
        ...
        instance_info = self.get_instance_info(coords_qv, labels_qv)
        ...
        return coords_qv, raw_coords_qv, feats_qv, labels_qv, labels_full_new, inverse_map, click_idx_qv, scene_name, num_obj, instance_info
...
def collation_fn(data_labels):
    coords, raw_coords, feats, labels, labels_full, inverse_map, click_idx, scene_name, num_obj, instance_info = list(zip(*data_labels)) 
    ...
    return coords_batch, raw_coords_batch, feats_batch, labels_batch, labels_full, inverse_map, click_idx, scene_name, num_obj, instance_info
